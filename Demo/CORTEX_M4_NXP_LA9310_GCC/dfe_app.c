...
static void prvTick( void *pvParameters, long unsigned int param1 )
{
  ...
  //PRINTF("%d : PMUX %d \r\n",ppsOutSkipCount, readPMUX(0));
	...
	/* RFNM's la9310_stats has no dbg1/dbg2 (debug SFN/slot counters); dropped
	 * to keep the RFNM 672-byte HIF layout the deployed driver hardcodes. */
	(void)ulCurrentSfn;
	(void)ulCurrentSlotInFrame;
...
static void prvProcessRx(struct dfe_msg *msg)
{
	...
	if(msg->type==0){
		log_err("BBDEV IPC receive invalid msg->type = 0\r\n");
		return;
	}
	...
	case DFE_TDD_SFN_SLOT_DELTA:
		...
		break;
#ifdef LIME_SDR_MICRO
	case DFE_LMS7002M_TX_CH_SET:
		set_lms7002m_tx_ch_set(msg->payload[0], /* tlms7002m_channel */
		msg->payload[1]  /* freq_Hz */
		);
	break;
	case DFE_LMS7002M_RX_CH_SET:
		set_lms7002m_rx_ch_set(msg->payload[0], /* lms7002m_channel */
		msg->payload[1]  /* freq_Hz */
		);
	break;
	case DFE_LMS7002M_TX_CH_SET_GAIN:
		set_lms7002m_tx_ch_set_gain(msg->payload[0], /* tlms7002m_channel */
		msg->payload[1]  /* gain_db */
		);
	break;
	case DFE_LMS7002M_RX_CH_SET_GAIN:
		set_lms7002m_rx_ch_set_gain(msg->payload[0], /* lms7002m_channel */
		msg->payload[1]  /* gain_db */
		);
	break;
#endif
